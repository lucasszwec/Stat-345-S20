...
i=1
j=0
cols.with.ab.zeros = function(my.mat, a, b) {
  for (i in 1:3) {
    zeros.per.column = sum(mat[,i] == 0)
    i.to.keep = a <= zeros.per.column && zeros.per.column <= b
    if (i.to.keep == FALSE) {
      i = i-j
      my.mat <- my.mat[,-i]
      j = j+1
      my.mat <- matrix(my.mat,nrow = 3,ncol = 3-j)
    }
  }
  return(my.mat)
}
...
cols.with.ab.zeros(mat, a=0, b=Inf) # Should get back original matrix
...
mat = diag(1, 3)
cols.with.ab.zeros(mat, a=2, b=2) # Should get back original matrix
```
...
# Output: the new list
list.extractor = function(my.list, i.to.keep, i.to.remove) {
  if (TRUE != is.null(i.to.keep)) {
    L = my.list[c(i.to.keep)]
    return(L)
  }
  if (TRUE != is.null(i.to.remove)) {
    L = my.list[c(-i.to.remove)]
    return(L)
  }
...
list.extractor(cool.list, i.to.keep=c(1,3), i.to.remove = NULL) # Should get list with ints, fracs
list.extractor(cool.list, i.to.keep=NULL, i.to.remove=4) # Should get list without bools
